...
hist <- ggplot(df, aes(x=Gender, y=Response, fill=Treatment))
hist + geom_bar(width = 0.4, position = position_dodge(width = 0.5), stat = "identity")
...
plot <- ggplot(df, aes(x=Gender, y=Response, fill=Treatment)) + 
  ...
  facet_wrap(~p) +
  mapply(function(w, pw)
    geom_bar(data = transform(df, p=sprintf("w_%.02f - pw_%.02f", w, pw)), stat = "identity", width= w, position = position_dodge(width = pw)),
    w = c(0.6,0.6,0.6,0.3,0.3,0.3), pw = c(0.4,0.7,1.0,0.4,0.7,1.0))
